...
"""One-shot Splunk setup for the live analytics dashboard (REST, port 8089).

Ensures the `dualcontext` index exists, enables HEC, creates/fetches a HEC token.
Prints `HEC_TOKEN=<token>`. Credentials from env:
...
TOKEN_NAME = "dualcontext"
INDEX = "main"  # dashboard queries index=main sourcetype=dualcontext_investigation
...
def enable_hec() -> None:
    ...
    if r.status_code not in (200, 201):
        print(f"  [!] HEC enable returned {r.status_code}")
        r.raise_for_status()
    print("  [+] HEC enabled (port 8088)")
